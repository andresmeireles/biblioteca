...
class ReturnBookTest extends TestCase
{
    ...
    public function testReturnBook(): void
    {
        ...
        $result = $this->returnBook->return($this->librarian, $borrow->id, (new \DateTime())->add(new \DateInterval('P2D')));
    ...
    public function testNonLibrarianReturn(): void
    {
        ...
        $result = $this->returnBook->return($this->user, $borrow->id, (new \DateTime())->add(new \DateInterval('P2D')));
    ...
    public function testPenalityForLessThan10Days(): void
    {
        ...
        $borrow = $this->borrow->borrow($book->id, $this->user, new \DateTime(), (new \DateTime())->add(new \DateInterval('P2D')));
        $this->borrow->changeApproveStatus($this->librarian, $borrow->id, true);
        $this->returnBook->return($this->librarian, $borrow->id, (new \DateTime())->add(new \DateInterval('P3D')));
    ...
    {
        $book = $this->addBook();
        $borrow = $this->borrow->borrow($book->id, $this->user, new \DateTime(), (new \DateTime())->add(new \DateInterval('P1D')));
        $this->borrow->changeApproveStatus($this->librarian, $borrow->id, true);
        $this->returnBook->return($this->librarian, $borrow->id, (new \DateTime())->add(new \DateInterval('P10D')));
        $canBorrow = CanBorrowBook::where('user_id', $this->user->id)->first()->can_borrow_at;
        $canBorrowDt = (new \DateTime($canBorrow))->format('d/m/Y');
        $canBorrowDate = (new \DateTime())->add(new \DateInterval('P2D'))->format('d/m/Y');
    ...
    public function testPenalityFor10Days(): void
    {
        ...
        $borrow = $this->borrow->borrow($book->id, $this->user, new \DateTime(), (new \DateTime())->add(new \DateInterval('P1D')));
        $this->borrow->changeApproveStatus($this->librarian, $borrow->id, true);
        $this->returnBook->return($this->librarian, $borrow->id, (new \DateTime())->add(new \DateInterval('P11D')));
        $canBorrow = CanBorrowBook::where('user_id', $this->user->id)->first()->can_borrow_at;
        $canBorrowDt = (new \DateTime($canBorrow))->format('d/m/Y');
    ...
    public function testPenalityForLessThan20Days(): void
    {
        ...
        $borrow = $this->borrow->borrow($book->id, $this->user, new \DateTime(), (new \DateTime())->add(new \DateInterval('P1D')));
        $this->borrow->changeApproveStatus($this->librarian, $borrow->id, true);
        $this->returnBook->return($this->librarian, $borrow->id, (new \DateTime())->add(new \DateInterval('P20D')));
        $canBorrow = CanBorrowBook::where('user_id', $this->user->id)->first()->can_borrow_at;
        $canBorrowDt = (new \DateTime($canBorrow))->format('d/m/Y');
        $canBorrowDate = (new \DateTime())->add(new \DateInterval('P7D'))->format('d/m/Y');
    ...
    public function testPenalityFor20Days(): void
    {
        ...
        $borrow = $this->borrow->borrow($book->id, $this->user, new \DateTime(), (new \DateTime())->add(new \DateInterval('P1D')));
        $this->borrow->changeApproveStatus($this->librarian, $borrow->id, true);
        $this->returnBook->return($this->librarian, $borrow->id, (new \DateTime())->add(new \DateInterval('P21D')));
        $canBorrow = CanBorrowBook::where('user_id', $this->user->id)->first()->can_borrow_at;
        $canBorrowDt = (new \DateTime($canBorrow))->format('d/m/Y');
    ...
    public function testPenalityForLessThan30Days(): void
    {
        ...
        $borrow = $this->borrow->borrow($book->id, $this->user, new \DateTime(), (new \DateTime())->add(new \DateInterval('P1D')));
        $this->borrow->changeApproveStatus($this->librarian, $borrow->id, true);
        $this->returnBook->return($this->librarian, $borrow->id, (new \DateTime())->add(new \DateInterval('P30D')));
        $canBorrow = CanBorrowBook::where('user_id', $this->user->id)->first()->can_borrow_at;
        $canBorrowDt = (new \DateTime($canBorrow))->format('d/m/Y');
        $canBorrowDate = (new \DateTime())->add(new \DateInterval('P14D'))->format('d/m/Y');
    ...
    public function testPenalityFor30Days(): void
    {
        ...
        $borrow = $this->borrow->borrow($book->id, $this->user, new \DateTime(), (new \DateTime())->add(new \DateInterval('P1D')));
        $this->borrow->changeApproveStatus($this->librarian, $borrow->id, true);
        $this->returnBook->return($this->librarian, $borrow->id, (new \DateTime())->add(new \DateInterval('P31D')));
        $canBorrow = CanBorrowBook::where('user_id', $this->user->id)->first()->can_borrow_at;
        $canBorrowDt = (new \DateTime($canBorrow))->format('d/m/Y');
    ...
    public function testPenalityForLessThan40Days(): void
    {
        ...
        $borrow = $this->borrow->borrow($book->id, $this->user, new \DateTime(), (new \DateTime())->add(new \DateInterval('P1D')));
        $this->borrow->changeApproveStatus($this->librarian, $borrow->id, true);
        $this->returnBook->return($this->librarian, $borrow->id, (new \DateTime())->add(new \DateInterval('P40D')));
        $canBorrow = CanBorrowBook::where('user_id', $this->user->id)->first()->can_borrow_at;
        $canBorrowDt = (new \DateTime($canBorrow))->format('d/m/Y');
        $canBorrowDate = (new \DateTime())->add(new \DateInterval('P21D'))->format('d/m/Y');
    ...
    public function testPenalityFor40Days(): void
    {
        ...
        $borrow = $this->borrow->borrow($book->id, $this->user, new \DateTime(), (new \DateTime())->add(new \DateInterval('P1D')));
        $this->borrow->changeApproveStatus($this->librarian, $borrow->id, true);
        $this->returnBook->return($this->librarian, $borrow->id, (new \DateTime())->add(new \DateInterval('P41D')));
        $canBorrow = CanBorrowBook::where('user_id', $this->user->id)->first()->can_borrow_at;
        $canBorrowDt = (new \DateTime($canBorrow))->format('d/m/Y');
    ...
    public function testPenalityForMoreThan40Days(): void
    {
        ...
        $borrow = $this->borrow->borrow($book->id, $this->user, new \DateTime(), (new \DateTime())->add(new \DateInterval('P1D')));
        $this->borrow->changeApproveStatus($this->librarian, $borrow->id, true);
        $this->returnBook->return($this->librarian, $borrow->id, (new \DateTime())->add(new \DateInterval('P80D')));
        $canBorrow = CanBorrowBook::where('user_id', $this->user->id)->first()->can_borrow_at;
        $canBorrowDt = (new \DateTime($canBorrow))->format('d/m/Y');
        $canBorrowDate = (new \DateTime())->add(new \DateInterval('P28D'))->format('d/m/Y');
    ...
    public function testEnsureBiggerPenality(): void
    {
        ...
        $borrow = $this->borrow->borrow($book->id, $this->user, new \DateTime(), (new \DateTime())->add(new \DateInterval('P1D')));
        $borrow2 = $this->borrow->borrow($book->id, $this->user, new \DateTime(), (new \DateTime())->add(new \DateInterval('P1D')));
        $this->borrow->changeApproveStatus($this->librarian, $borrow->id, true);
        $this->borrow->changeApproveStatus($this->librarian, $borrow2->id, true);
        $this->returnBook->return($this->librarian, $borrow->id, (new \DateTime())->add(new \DateInterval('P80D')));
        $this->returnBook->return($this->librarian, $borrow2->id, (new \DateTime())->add(new \DateInterval('P2D')));
    ...
    public function testLossBook(): void
    {
        ...
        $borrow = $this->borrow->borrow($book->id, $this->user, new \DateTime(), (new \DateTime())->add(new \DateInterval('P1D')));
        $this->borrow->changeApproveStatus($this->librarian, $borrow->id, true);
        $this->returnBook->return($this->librarian, $borrow->id, isLost: true);
        $canBorrow = CanBorrowBook::where('user_id', $this->user->id)->first()->can_borrow_at;
        $canBorrowDt = (new \DateTime($canBorrow))->format('d/m/Y');
        $canBorrowDate = (new \DateTime())->add(new \DateInterval('P28D'))->format('d/m/Y');
